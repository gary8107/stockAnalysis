...
// Tab 顯示日期；如果該區段有有意義副標（例如「盤前 · 追高警告場」），會顯示成第二行。
...
class DateTabBar extends StatelessWidget implements PreferredSizeWidget {
  const DateTabBar({super.key, required this.sections});

  final List<MarkdownSection> sections;

  // 同檔案內任一區段有副標就把 TabBar 撐成兩行高（保持節奏一致，否則 Tab 高度不一）
  bool get _anyHasNote => sections.any((s) => s.displayNote != null);
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: TabBar(
        ...
        tabs: sections
            .map((s) => _DateTab(section: s, expandedHeight: _anyHasNote))
            .toList(),
      ),
    );
  }
...
class _DateTab extends StatelessWidget {
  const _DateTab({required this.section, required this.expandedHeight});
  ...
  @override
  Widget build(BuildContext context) {
    final displayNote = section.displayNote;
